...
def main_menu():
    ...
    markup.row("➕ Добавить слово", "🗑 Удалить слово", "✏️ Редактировать")
    return markup
...
def start(message):
    bot.send_message(
        message.chat.id,
        "*Привет!*\n\n"
        "Я бот-справочник. Напиши слово или фразу — я найду подходящий абзац.\n\n"
        "Используй меню ниже или команды:\n"
        "`/add слово1,слово2 текст_абзаца`\n"
        "`/delete слово1,слово2`",
...
def add_word(message):
    ...
    if msg_text.startswith("➕"):
        bot.send_message(message.chat.id,
                         "Используй формат:\n`/add слово1,слово2 текст_абзаца`\n"
                         "Текст абзаца может быть с переносами строк.")
        return
    text = msg_text[len("/add "):].strip()
    if " " not in text:
        bot.reply_to(message, "❗️ Формат: `/add слово1,слово2 текст_абзаца`")
        return
    ...
    save_data(answers)
    bot.reply_to(message, f"✅ Слова *{', '.join(keys)}* добавлены!")
...
def delete_word(message):
    ...
    reply = ""
    if deleted:
        reply += f"✅ Слова *{', '.join(deleted)}* удалены!\n"
    if not_found:
        reply += f"⚠️ Слова *{', '.join(not_found)}* не найдены."
...
def edit_word(message):
    ...
    if msg_text.startswith("✏️"):
        bot.send_message(message.chat.id,
                         "Используй формат:\n`/edit ключ1,ключ2 Новый текст абзаца`")
        return
    text = msg_text[len("/edit "):].strip()
    if " " not in text:
        bot.reply_to(message, "❗️ Формат: `/edit ключ1,ключ2 Новый текст абзаца`")
        return
